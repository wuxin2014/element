...
export default {
  ...
  methods: {
    ...
    queryHistoryData(params) {
      // queryCzceHistoryData({
      //   ...params,
      //   fund_account: this.sysInfo.fund_account,
      //   exchange_code: this.searchInfo.exchange_code
      // }).then(res => {
      //   if (res.code === '000000') {
      //     if (params.param_type === 'HEDGE_CZCE_CONTRACT_BUYER') {
      //       this.contractBuyerList = res.data || []
      //     } else {
      //       this.contractSellerList = res.data || []
      //     }
      //   }
      // })
    },
...
.contract-online-fill-czce-dialog {
  .dialog-title-text {
    color: transparent;
  }
  .tips {
