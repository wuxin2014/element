<template>
  <div class="form-container">
    ...
    <el-form
      ...
      size="small" 
      label-position="top">
      <el-form-item prop="sellerDeliveryInformations" style="padding-bottom: 16px">
        <div>
          <el-table
            border
            :data="renderList"
...
<script>
import AddDailog from './SellerFocusAddDailog.vue'
let uuid = 1
export default {
  props: {
    isDetail: {
  ...
  data() {
    return {
      ...
      rules: {
        standardWeight: { required: true, message: '请先添加表格内容', trigger: 'change' },
        payment: { required: true, message: '请先添加表格内容', trigger: 'change' },
        sellerDeliveryInformations: { required: true, message: '请点击添加', trigger: 'change' },
      },
      pageNum: 1,
      pageSize: 10,
  ...
  methods: {
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        ...
        if (['warehouse'].includes(column.property)) {
          sums[index] = '--/--'
          return
        }
